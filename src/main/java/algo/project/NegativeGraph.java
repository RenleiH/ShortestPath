...
public class NegativeGraph extends Graph {
     ...
     */
    public NegativeGraph(String filePath) {
        super();  // Call the superclass (Graph) constructor to initialize the graph.
     ...
     */
    public NegativeGraph(NegativeGraph originalGraph) {
        super();  // Call the superclass (Graph) constructor to initialize the graph.

        // Copy vertices from the original graph.
        for (Vertex vertex : originalGraph.getAllVertices().values()) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (parts.length == 3) {  // Ensure there are exactly three parts (from, to, weight).
                    ...
                    Vertex fromVertex = this.getVertexByName(fromName);
                    // Create a new vertex if not present. Latitude, Longitude, GridRow, and GridCol are 0, not useful here.
                    if (fromVertex == null) {
                        fromVertex = new Vertex(fromName, VertexType.PICKUP, 0, 0, 0, 0);
                        this.addVertex(fromVertex);  // Add the vertex to the graph.
                    }

                    // Get or create the destination vertex.
                    Vertex toVertex = this.getVertexByName(toName);
                    // Create a new vertex if not present. Latitude, Longitude, GridRow, and GridCol are 0, not useful here.
                    if (toVertex == null) {
                        toVertex = new Vertex(toName, VertexType.DROPOFF, 0, 0, 0, 0);
    ...
    private void ensureBidirectionalEdges() {
        List<Vertex> vertices = this.getAllVertices().values().stream().toList();  // Get all vertices in the graph.
        for (Vertex v1 : vertices) {  // Iterate over each vertex.
            for (Vertex v2 : vertices) {  // Compare with every other vertex.
                if (!v1.equals(v2)) {  // Avoid self-loops.
                    ...
                    if (!hasEdgeFromV1ToV2) {
                        this.addEdge(new Edge(v1, v2, 0));  // Add edge from v1 to v2.
                    }
                    // If no edge exists from v2 to v1, add a bidirectional edge with weight 0.
                    if (!hasEdgeFromV2ToV1) {
                        this.addEdge(new Edge(v2, v1, 0));  // Add edge from v2 to v1.
                    }
                }
            }
        }
    ...
    public static org.graphstream.graph.Graph toGraphStreamGraph(Graph customGraph) {
        ...
        for (Vertex vertex : customGraph.getAllVertices().values()) {
    ...
    public Edge getEdge(Vertex fromVertex, Vertex toVertex) {
        ...
        for (Edge edge : this.getEdges()) {
            // Check if the edge connects the two specified vertices
            if (edge.getVertexF().equals(fromVertex) && edge.getVertexT().equals(toVertex)) {
