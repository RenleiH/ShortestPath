...
public class Graph {
    ...
    Map<String, Vertex> allVertices;  // Map to store vertices by their names
    Map<VertexType, ArrayList<Vertex>> differentVertices;  // Map to store vertices by their type (e.g., normal, garage, pickup, delivery)
    ArrayList<Edge> edges;  // List to store all edges in the graph
     ...
     */
    public ArrayList<Vertex> getVerticesByType(VertexType type) {
     ...
     */
    public Vertex getVertexByRowAndCol(int row, int col) {
     ...
     */
    public Graph copy() {
        Graph copy = new Graph();  // Create a new graph

        for (Vertex vertex : allVertices.values()) {  // Copy each vertex
            copy.addVertex(vertex.copy());
        }

        for (Map.Entry<VertexType, ArrayList<Vertex>> entry : differentVertices.entrySet()) {  // Copy each list of vertices by type
        ...
        for (Edge edge : edges) {  // Copy each edge
    ...
    public String toString() {
        ...
        for (Map.Entry<VertexType, ArrayList<Vertex>> entry : differentVertices.entrySet()) {
